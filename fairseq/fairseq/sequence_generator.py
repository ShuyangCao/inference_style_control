        ...
        **kwargs
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            lprobs, avg_attn_scores = model.forward_decoder(
                tokens[:, :step + 1], encoder_outs, temperature=self.temperature, alignment_layer=self.alignment_layer
            )
            ...
            if avg_attn_scores is not None:
                if attn is None:
                    ...
                    # attn = scores.new(bsz * beam_size, avg_attn_scores.size(1), src_tokens.size(1), max_len + 2)
                    attn_buf = attn.clone()
            ...
            if len(finalized_sents) > 0:
                ...
                if attn is not None:
                    ...
                    # attn = attn.view(bsz, -1)[batch_idxs].view(new_bsz * beam_size, attn.size(1), attn.size(2), -1)
...
class EnsembleModel(torch.nn.Module):
    ...
    @torch.no_grad()
    def forward_decoder(self, tokens, encoder_outs, temperature=1., alignment_layer=None):
    ...
    def _decode_one(
        ...
        temperature=1., alignment_layer=None
    ):
        if self.incremental_states is not None:
            decoder_out = list(model.forward_decoder(
                tokens, encoder_out=encoder_out, incremental_state=self.incremental_states[model], alignment_layer=alignment_layer
            ))
        else:
...
class SequenceGeneratorWithAlignment(SequenceGenerator):
    ...
    def generate(self, models, sample, **kwargs):
        ...
        if any(getattr(m, 'full_context_alignment', False) for m in model.models):
            ...
            # ]
            attn = [
                finalized[i // beam_size][i % beam_size]['attention'].transpose(2, 1)
                for i in range(bsz * beam_size)
            ]

        num_head = attn[0].size(0)
        # Process the attn matrix to extract hard alignments.
        for i in range(bsz * beam_size):
            # alignment = utils.extract_hard_alignment(attn[i], src_tokens[i], tgt_tokens[i], self.pad, self.eos)
            alignment = [utils.extract_hard_alignment(attn[i][j], src_tokens[i], tgt_tokens[i], self.pad, self.eos) for j in range(num_head)]
...
class EnsembleModelWithAlignment(EnsembleModel):
    ...
    def _decode_one(
        ...
        if self.incremental_states is not None:
            ...
            decoder_out = list(model.forward_decoder(tokens, encoder_out=encoder_out, alignment_layer=alignment_layer))
        decoder_out[0] = decoder_out[0][:, -1:, :]
        if temperature != 1.:
            decoder_out[0].div_(temperature)
        attn = decoder_out[1]
