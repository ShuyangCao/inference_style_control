...
class StateAdjustSequenceGenerator(object):
    def __init__(
        ...
        Token level LM update the hidden state of the current token
    ...
    def _generate(
        ...
        if 'source_label' in sample:
            encoder_target = sample['source_label']
            encoder_target = encoder_target.eq(0.)
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                ...
                style_model.decoder.reorder_incremental_state(incremental_state, reorder_state)
    ...
    def _decode(self, cclm, gen_model, tokens, encoder_out, incremental_state):
        """
        discriminator is a CCLM
        """
        b_sz, seq_len = tokens.size()

        # LM use EOS as BOS, so we take the tokens directly
        with torch.no_grad():
            first_x, _, _, _ = gen_model.decoder.extract_to_layer(
                tokens, encoder_out, incremental_state
            )
            cclm_tokens = torch.cat([tokens[:, :1], tokens.new_full((b_sz, 1), self.target_label), tokens[:, 1:]], dim=-1)
            if seq_len == 1:
                _, _ = cclm(cclm_tokens[:, :1], incremental_state=incremental_state)
            first_target_logits, _ = cclm(cclm_tokens, incremental_state=incremental_state)
        ...
        pred_logits = logits  # bsz x 1 x vsz
        pert_lprob = None
        for i in range(self.iteration):
            sharp_prob = torch.softmax(logits / 0.5, dim=-1)[:, -1:]
            tokens_clone = tokens.clone()
            cclm_tokens_clone = cclm_tokens.clone()
            # target_logits = first_target_logits
            target_probs = first_target_probs
            for future_i in range(self.future_window):
                tokens_clone = torch.cat([tokens_clone, tokens_clone[:, -1:]], dim=1)
                cclm_tokens_clone = torch.cat([cclm_tokens_clone, cclm_tokens_clone[:, -1:]], dim=1)
                ...
                with torch.no_grad():
                    inputs_embed = torch.matmul(sharp_prob, cclm.decoder.embed_tokens.weight)
                    lm_logits, _ = cclm(cclm_tokens_clone, incremental_state=incremental_state, inputs_embed=inputs_embed)
                    lm_probs = torch.softmax(lm_logits, dim=-1)
                    # target_logits = torch.cat([target_logits, lm_logits], dim=1)
                    target_probs = torch.cat([target_probs, lm_probs], dim=1)
                sharp_prob = torch.softmax(logits / 0.5, dim=-1)[:, -1:]
                pred_logits = torch.cat([pred_logits, logits], dim=1)

            disc_loss = torch.sum(- target_probs * F.log_softmax(pred_logits, dim=-1), dim=-1)  # bsz x (1 + future)
            disc_loss = torch.mean(disc_loss, dim=-1)  # bsz
            all_loss = disc_loss
            ...
            if self.future_window > 0:
                ...
                # the LM decoder
                for layer in cclm.decoder.layers:
                    self_state = layer.self_attn.get_incremental_state(incremental_state, 'attn_state')
                    for k, v in self_state.items():
                        if k == 'prev_key_padding_mask':
                            self_state[k] = v[:, :-self.future_window] if v is not None else None
